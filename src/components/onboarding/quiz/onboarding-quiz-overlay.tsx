...
export function OnboardingQuizOverlay() {
  ...
  // Auto-complete if all questions already answered, or skip if none are seeded
  useEffect(() => {
    if (loading || completion) return;
    if (questions.length === 0) {
      // No diagnostic questions in DB — advance onboarding state and skip to schedule
      fetch("/api/quiz/complete", { method: "POST" }).finally(() => {
  ...
  const question = questions[currentIndex];
  if (!question) {
    // questions empty — show spinner while the redirect effect fires
    const redirecting = (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-background">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
      </div>
    );
    return createPortal(redirecting, document.body);
  }
