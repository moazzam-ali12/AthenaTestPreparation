...
// triggerOnly=true renders just the "Edit" button (pencil + label) that opens the same dialog,
// without the surrounding card and compact schedule summary. Use in sidebars/tabs where the
// schedule is already displayed in full.
export function ScheduleEditor({ triggerOnly = false }: { triggerOnly?: boolean } = {}) {
  ...
  const timeLabel = schedules[0] ? formatTimeStr(schedules[0].startTime) : null;

  // Shared dialog — wraps both the trigger and the content regardless of mode
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      {triggerOnly ? (
        // Minimal trigger: just a pencil + "Edit" label, no card wrapper
        ...
        // Full card widget with compact schedule summary
      ...
      {/* Shared dialog content — rendered once, used by both trigger modes */}
