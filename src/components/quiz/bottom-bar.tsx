...
export function BottomBar({
  ...
  // Once everything's answered, let the user submit from wherever they're
  // reviewing instead of forcing them back to the literal last question.
  const showEarlySubmit = !onFinish && !isLast && unansweredCount === 0;
  ...
  const handleReviewAnswers = () => {
    ...
    for (let i = 0; i < total; i++) {
      if (getStatus(i) !== "answered") {
        onGoTo(i);
        return;
      }
    }
    // Nothing unanswered/marked — go back to the start for a full review pass.
    onGoTo(0);
  };
      ...
      <div className="flex h-14 shrink-0 items-center justify-between border-t bg-card px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isLast && (
            <Button
              size="sm"
              variant={showEarlySubmit ? "outline" : "default"}
              onClick={onNext}
          ...
          {(isLast || showEarlySubmit) && (
            <Button
              size="sm"
              onClick={handleSubmitClick}
              disabled={isLast ? nextDisabled : false}
            >
