...
import { QuestContext, type QuestCompletionData } from "./quest-context";
...
export function QuestProvider({ quest, problems, children }: Props) {
  ...
  const [phase, setPhase] = useState<"active" | "canvas" | "completed">(
  ...
  // Track when the current question was first shown so responseTimeMs is accurate
  ...
  // Combine DB-backed isCorrect (from initial load) with lockedIds (answered correctly this session)
  const actualScore = problems.filter((p) => lockedIds.has(p.id) || p.isCorrect === true).length;
    ...
    (problemId: string, optionIndex: number) => {
      ...
      if (isCorrect) {
        ...
        answerMutation.mutate({
          ...
          isCorrect: true,
          responseTimeMs: Math.round(performance.now() - questionStartTimeRef.current),
  ...
  const handleComplete = useCallback(() => {
    ...
    completeMutation.mutate(
      { questId: quest.id, timeElapsedSeconds: elapsed },
      {
        onSuccess: (data) => {
          setCompletionData({
            scores: data.scores,
