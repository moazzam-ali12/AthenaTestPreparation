...
const DISCIPLINE_SECTIONS: { code: "BAR" | "ISC" | "TCP"; name: string; desc: string }[] = [
  {
    code: "BAR",
    name: "Business Analysis and Reporting",
    desc: "Financial statement analysis, data analytics, and managerial accounting.",
  },
  {
    code: "ISC",
    name: "Information Systems and Controls",
    desc: "IT audit, data management, and system/organization controls.",
  },
  {
    code: "TCP",
    name: "Tax Compliance and Planning",
    desc: "Individual and entity tax compliance, and tax planning strategies.",
  },
];
...
interface OnboardingData {
  ...
  targetDisciplineSection: "BAR" | "ISC" | "TCP" | null;
}
...
export default function OnboardingPlanPage() {
  ...
  const canContinue = useCallback(() => {
    switch (step) {
      ...
      case 5: return data.targetDisciplineSection !== null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-20 flex w-full max-w-lg flex-col items-center px-6 text-center">
          ...
          >
            ...
            {step === 5 && (
              <>
                ...
                <p className="mb-1 text-white/70">
                  AUD, FAR, and REG are required for every candidate. Now pick the one
                  discipline section you&apos;ll specialize in.
                </p>
                <p className="mb-8 font-semibold text-white">Pick the section that fits your career path.</p>
                <div className="flex flex-col gap-3">
                  {DISCIPLINE_SECTIONS.map((section) => (
                    <button
                      key={section.code}
                      className={`rounded-xl border px-5 py-4 text-left transition-all ${
                        data.targetDisciplineSection === section.code
                          ? "border-white/40 bg-white/20"
                          : "border-white/15 bg-black/40 hover:border-white/25 hover:bg-black/50"
                      }`}
                      onClick={() => setData({ ...data, targetDisciplineSection: section.code })}
                    >
                      <p className="text-sm font-semibold text-white">
                        {section.code} &mdash; {section.name}
                      </p>
                      <p className="mt-1 text-xs text-white/70">{section.desc}</p>
                    </button>
                  ))}
                </div>
              </>
        ...
        <div className="mt-10 flex items-center gap-6">
          ...
          >
            {submitting ? "Saving..." : step === 5 ? "Let\u2019s Go" : "Continue"}
          </button>
