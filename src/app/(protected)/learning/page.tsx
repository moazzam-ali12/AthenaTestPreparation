...
type Topic = {
  ...
  cpaRelevance: { percentageOfTest: number };
  subtopics: { id: string }[];
  subject: string;
  locked: boolean;
};
...
export default function LearningPage() {
  const [activeSubject, setActiveSubject] = useState<string>("aud");
        ...
        {filteredTopics.map((topic) => {
          ...
          const cardContent = (
            <Card
              className={cn(
                "transition-colors",
                topic.locked ? "opacity-50" : "hover:bg-accent/30"
              )}
            >
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {topic.cpaRelevance.percentageOfTest}% of the CPA Exam
                  </span>
                </div>
              </CardContent>
            </Card>
