...
const CPA_SECTIONS: SectionCategory[] = ["AUD", "FAR", "REG", "BAR", "ISC", "TCP"];

/** Maps a topic's `subject` (e.g. "aud") to its CPA section category (e.g. "AUD"). */
export function subjectToSectionCategory(subject: string): SectionCategory {
  const upper = subject.toUpperCase();
  if ((CPA_SECTIONS as string[]).includes(upper)) return upper as SectionCategory;
  throw new Error(`Unknown CPA subject: ${subject}`);
}
  ...
  const toInsert = subtopics
    ...
    .map((s) => {
      ...
      return {
        user_id: userId,
        subtopic_id: s.id,
        section_category: subjectToSectionCategory(topic?.subject ?? "aud"),
      };
    });
