...
export async function generateQuestForDate(
  ...
  // Get bucket assignments and question distribution. CPA has no composite
  // score, so difficulty is driven purely by each subsection's own mastery
  // level (no global difficulty floor).
  const buckets = composeDailyQuestBuckets(adjustedSkills);
  ...
  for (const entry of distribution) {
    ...
    // Find problems near the target difficulty, widening the window (±1, ±2, ...)
    // until enough unseen candidates turn up or the full 1-10 range is covered.
    // The seeded difficulty floor is discrete (easy=3/medium=5/hard=8), so a
    // fixed ±1 window can come up empty for skill levels between those tiers.
    let candidates: { id: string; difficulty_level: number }[] = [];
    let available: { id: string; difficulty_level: number }[] = [];

    for (let widen = 1; widen <= 9; widen++) {
      const minDiff = Math.max(1, entry.targetDifficulty - widen);
      const maxDiff = Math.min(10, entry.targetDifficulty + widen);
      ...
      candidates = data ?? [];
      available = candidates.filter((c) => !recentProblemIds.has(c.id));

      if (available.length >= entry.questionCount) break;
      if (minDiff === 1 && maxDiff === 10) break; // covered the full range
    }
