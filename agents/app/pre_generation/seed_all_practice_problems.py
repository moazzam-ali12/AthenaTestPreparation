...
Usage:
    ...
    cd agents && python seed_all_practice_problems.py --subject far
...
from app.pre_generation.content_workflow import (
    AUD_CONTENT_MAP,
    FAR_CONTENT_MAP,
    REG_CONTENT_MAP,
    BAR_CONTENT_MAP,
...
async def main():
    ...
    parser.add_argument(
        "--subject",
        default="aud",
        choices=["aud", "far", "reg", "bar", "isc", "tcp", "all"],
        help="CPA Exam section to seed (default: aud)",
    )
    parser.add_argument(
        "--count",
        type=int,
    ...
    subjects = (
        list(CONTENT_MAPS.items())
        if args.subject == "all"
        else [(args.subject, CONTENT_MAPS[args.subject])]
    )
